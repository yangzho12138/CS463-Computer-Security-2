...
def transformation_stat(processed_data_file, output_file):
    ...
    for rule in rules:
        rule_transformation_in_order[rule] = {}
    
    # record the frequency of each transformation of each rule
    frequency = [{}, {}, {}, {}, {}, {}, {}]
    ...
    for i in range(len(data)):
        ...
        if rule == 'seqkey':
            ...
            if transformation in frequency[rules.index(rule)]:
                frequency[rules.index(rule)][transformation] += 1
            else:
                frequency[rules.index(rule)][transformation] = 1
    
    # sort the transformation by frequency
    for i in range(len(frequency)):
        rule_transformation_in_order[rules[i]] = sorted(frequency[i], key=frequency[i].get, reverse=True)
