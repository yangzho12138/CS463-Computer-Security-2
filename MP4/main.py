...
def stat(svm_model):
    ##############TODO: implement me #########
    # how many samples were used in training
    train_cnt = svm_model.X_train.shape[0]
    # how many samples were used in testing
    test_cnt = svm_model.X_test.shape[0]
    # how many malware and goodware in the training set
    train_mal_cnt = np.sum(svm_model.y_train == 1)
    train_benign_cnt = np.sum(svm_model.y_train == 0)
    # how many malware and goodware in the testing set
    test_mal_cnt = np.sum(svm_model.y_test == 1)
    test_benign_cnt = np.sum(svm_model.y_test == 0)
